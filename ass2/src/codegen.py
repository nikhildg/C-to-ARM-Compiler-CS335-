...
tackeywords = ['ifgoto', 'goto', 'return', 'call', 'print', 'label', 'function' , 'exit' ,
                '<=', '>=', '==', '>', '<', '!=', '=', 
                '+', '-', '*', '/', '%', 
                '&&', '||', '~' ]
mathops = ['+', '-', '*', '/', '%']
...
def translate(instruction):
    ...
    if operator in mathops:
        ...
        if operator == '+':
            if isnumber(operand1) and isnumber(operand2):
                ...
                regdest = getReg(result, line, assembly)
    ...
    # Generating code for assignment operations
        ...
        regdest = getReg(destination, line, assembly)
