...
def p_invocation_expression(p):
    ...
    if func_name == None:
        if p[1]['value']=="Console.WriteLine":
            p[0]['code'] = []
            for i in range(len(p[3])-1):
                var = symbol_table.lookup(p[3][i+1]['value'],symbol_table.curr_table)
                if 'uppertype' in p[3][i+1] and p[3][i+1]['uppertype'] == 'literal':
                        pass
                elif var == None or var['uppertype']=='function' :
                    print("ERROR L", ": ", p[3][i+1]['value'], " has not been declared as variable")
                    print("Compilation Terminated")
                    exit()
                p[0]['code'] += ["print, "+p[3][i+1]['value']]
        else:
        ...
        if p[3]!=None and len(p[3])>0:
            ...
            for arg in p[3]:
                ...
                if 'uppertype' in arg and arg['uppertype']=='literal':
                        pass
                elif var == None or var['uppertype']=='function' :
...
def p_compilation_unit(p):
    ...
    if len(p)==2:
        ...
        p[0] = p[2]
    line_no = 1
    print (str(line_no) + ", call, Main, t00")
    line_no = line_no + 1
...
def p_method_declaration(p):
    ...
    if p[1]['parameters'] != None:
        for param in reversed(p[1]['parameters']):
...
def p_method_header(p):
    ...
    if p[5] != None:
        for param in p[5]:
            symbol_table.insert_variable(param['type'], param['value'])
